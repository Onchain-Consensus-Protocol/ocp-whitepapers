...
| 功能 | 状态 | 说明 |
|------|------|------|
| 金库投票决定结果 | ✅ | `PredictionMarketVault`：投票=是，不投票=否，占比决定 outcome |
...
- `PredictionMarketVault`（`core/PredictionMarketVault.sol`）
- `PredictionMarketVaultFactory`（`factory/PredictionMarketVaultFactory.sol`）
...
| 功能 | 文档 | 说明 |
|------|------|------|
| 挑战机制（一次性） | CCP 白皮书 §2 | 1 小时挑战窗口，仅落后方可发起挑战 |
| 保证金 | CCP 白皮书 §2 | 挑战需支付 b，赢退输给对手 |
| 最终对决期 | CCP 白皮书 §2 | 挑战后 PM+金库开放 1 小时，新资金可流入 |
| **一次性挑战** | CCP 白皮书 §2 | 每场事件仅允许一次挑战，对决期后终局（无多轮） |
...
| **CAPITAL_COMMITMENT_PROTOCOL_WHITEPAPER_CN.md** | CCP 机制，质押—挑战—共识 |
